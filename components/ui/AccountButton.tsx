...
export function AccountButton() {
    ...
    return (
      ...
      >
        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-ink text-[10px] font-bold text-white">
          {levelFromPoints(profile.total_score)}
        </span>
  ...
  return (
    <Link
      href="/profile"
      className="ink-shadow-sm wonky whitespace-nowrap border-[2.5px] border-ink bg-cta px-4 py-1.5 text-xs font-bold text-background transition-all hover:opacity-95 active:translate-x-[2px] active:translate-y-[2px] active:shadow-none"
    >
      Sign up
    </Link>
  );
